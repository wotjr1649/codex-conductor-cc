...
Execution rules:
- Put every flag before the task text. Option parsing stops at the first word of the prompt, so a flag placed after it is kept as prompt text and has no effect.
...
- `--effort`: accepted values are `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. Codex decides which of those a given model accepts and refuses the rest.
