...
test("P6-RUNTIME-ROOT-001 requires a canonical current-UID 0700 directory", () => {
  const valid = {
    ...
    uid: 1000,
    mode: 0o40700,
    canonicalPath: "/run/user/1000/codex-conductor-1000",
    expectedPath: "/run/user/1000/codex-conductor-1000",
    expectedUid: 1000
  };
  ...
  for (const patch of [
    ...
    { mode: 0o40750 },
    { mode: 0o40707 },
    { canonicalPath: "/private/run/user/1000/codex-conductor-1000" }
  ]) {
...
test("P6-RUNTIME-ROOT-002 creates and validates the private POSIX root", {
  ...
  assert.equal(root, path.join(fs.realpathSync.native(baseDir), `codex-conductor-${uid}`));
